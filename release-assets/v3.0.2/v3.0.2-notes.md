...
- `Plus` as a material modifier
- `Matte` as the visual / filament variant
...
- If `App labels` still show the expected label, Spool Studio can still reconstruct the user-facing filament name.
- If `SpoolLink` shows different data, the difference comes from what the firmware currently reports.
- This makes it easier to tell whether a mismatch is in the app data, Spoolman, or the printer firmware / Fluidd integration.
...
- `card_uids` support for UID-based tag linking
- `material_modifier` support for preserving values such as `Plus` or `HS`
- access to the legacy Spoolman material conversion tool
...
- RFID bridge status
- availability of the required `SET_SPOOL_ID` assignment command
