...
fun httpUrlValidationError(url: String): String? = when {
    url.isBlank() -> "URL fehlt"
    !url.startsWith("http://") && !url.startsWith("https://") -> "URL muss mit http:// oder https:// beginnen"
    else -> null
}

fun connectionErrorMessage(error: Exception): String = when {
    error.message?.contains("timeout", true) == true -> "Timeout"
    error.message?.contains("Unable to resolve host", true) == true -> "Host nicht erreichbar"
    else -> "Fehler: ${error.message ?: "Unbekannt"}"
}
